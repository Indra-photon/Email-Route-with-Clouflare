...
function parseDomain(input: string): { clean: string } | { error: string } {
  ...
  const valid = /^[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z]{2,})+$/.test(s);
  if (!valid || !s) {
    return { error: 'Use plain domain format, e.g. "example.com" or "mail.example.co.uk"' };
  }
  return { clean: s };
}
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 items-start">
        <div className="flex flex-col gap-1">
          <input
            ...
            disabled={isBusy}
            value={value}
            onChange={(e) => { setValue(e.target.value); setDomainError(null); }}
            className={`w-64 rounded-lg border bg-white dark:bg-neutral-800 px-3 py-2 text-sm text-neutral-900 dark:text-neutral-100 placeholder-neutral-400 transition-colors duration-100 outline-none focus:outline-none [box-shadow:none] focus:[box-shadow:none] disabled:opacity-50 disabled:cursor-not-allowed ${
              domainError
