...
interface UsageBarProps {
  label: string;
  current: number;
  /** -1 = unlimited (∞), 0 = not tracked (no cap shown), positive = hard limit */
  max: number;
  className?: string;
}
...
export function UsageBar({ label, current, max, className = "" }: UsageBarProps) {
  ...
  const isNotTracked = max === 0;
  const percent = isUnlimited || isNotTracked ? 0 : Math.min(100, Math.round((current / max) * 100));
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      ...
      {!isUnlimited && !isNotTracked && percent >= 80 && (
