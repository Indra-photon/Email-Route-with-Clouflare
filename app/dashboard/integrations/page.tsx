...
export default function IntegrationsPage() {
  ...
  // Show toasts for Slack OAuth result redirected back from /callback
  useEffect(() => {
  ...
  const typeIcon  = (i: Integration) => i.type === "discord" ? "🎮" : "💬";
  const typeBadge = (i: Integration) => i.type === "discord"
    ? "bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300"
    : "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300";

  const displayUrl = (i: Integration) => {
    if (i.type === "slack" && i.authMethod === "oauth") {
      return i.slackTeamName && i.slackChannelName
        ? `${i.slackTeamName} · #${i.slackChannelName}`
        : "Connected via Slack OAuth";
    }
    return truncate(i.webhookUrl);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
          Connect Slack or Discord — used as routing targets for aliases.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* ── Slack OAuth card ─────────────────────────────────────── */}
        <div className="flex flex-col gap-3 bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 mb-1">
            💬 Connect Slack
          </h2>
          <p className="text-xs text-neutral-500 dark:text-neutral-400">
            Emails are posted to your chosen Slack channel. Replies in the thread are automatically sent back as emails — no app setup needed on your side.
          </p>
          <a href="/api/integrations/slack/oauth">
            <Button className="mt-1 self-start bg-[#4A154B] hover:bg-[#611f69] text-white">
              Add to Slack
            </Button>
          </a>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 mb-1">🎮 Add Discord Webhook</h2>

          <input type="hidden" value="discord" readOnly />
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-neutral-50 dark:bg-neutral-800">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-medium text-neutral-600 dark:text-neutral-400">Target</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-neutral-500">No integrations yet — connect Slack or add a Discord webhook above.</td></tr>
            ) : (
              integrations.map((i) => (
                <tr key={i.id} className="border-t border-neutral-200 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${typeBadge(i)}`}>
                      {typeIcon(i)} {i.type}
                      {i.type === "slack" && i.authMethod === "oauth" && (
                        <span className="ml-1 opacity-70">· OAuth</span>
                      )}
                    </span>
                  </td>
