  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex flex-wrap gap-3 items-end">
        ...
        />
        <div className="flex flex-col space-y-1">
          <label className="text-sm font-schibsted text-neutral-700 dark:text-neutral-300">Welcome Message</label>
          <input
            type="text"
            value={welcomeMessage}
            onChange={(e) => setWelcomeMessage(e.target.value)}
        ...
        <div className="flex flex-col space-y-1">
          <label className="text-sm font-schibsted text-neutral-700 dark:text-neutral-300 flex items-center gap-1">
            <IconPalette size={13} />
            Color
          </label>
          <div className="flex items-center gap-1.5 h-[38px]">
...
export default function ChatWidgetsDashboard() {
  ...
  const [domains, setDomains] = useState<Domain[]>([]);
  const [loading, setLoading] = useState(true);
  const [activePageTab, setActivePageTab] = useState<PageTab>("widgets");
  const [newKey, setNewKey] = useState<{ key: string; script: string } | null>(null);
